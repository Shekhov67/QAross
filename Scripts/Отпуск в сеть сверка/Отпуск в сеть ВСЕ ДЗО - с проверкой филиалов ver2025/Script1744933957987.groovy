...
def runTestScenario(testNum) {
	try {
		...
		if (!WebUI.verifyElementPresent(findTestObject(path), 10, FailureHandling.OPTIONAL)) {
			...
			WriteToExcel2(err: "Нет данных", testNum: testNum)
			WebUI.closeBrowser()
			return
		}
		...
		// Сравнение данных
		boolean check = Check(
			typeData: 'Отпуск в сеть',
			pageString: pageDataString,
			fileString: otpuskVSeti,
			path: path,
			testNum: testNum
		)
		...
		WriteToExcel2(err: "Ошибка выполнения: ${e.getMessage()}", testNum: testNum)
		WebUI.closeBrowser()
	}
}
 ...
 */
def selectCompany(testNum) {
	def companies = [
		1: 'выбрать АО Тываэнерго',
		2: 'выбрать АО Чеченэнерго',
		3: 'выбрать Россети Волга',
		4: 'выбрать Россети Кубань',
		5: 'выбрать Росссети Ленэнерго(ГК)',
		6: 'выбрать Россети Московский регион',
		7: 'выбрать Россети Северный Кавказ(ГК)',
		8: 'выбрать Россети Северо-Запад',
		9: 'выбрать Россети Сибирь(ГК)',
		10: 'выбрать Россети Томск',
		11: 'выбрать Тюмень',
		12: 'выбрать Россети Урал(ГК)',
		13: 'выбрать РаспредКомплекс',
		14: 'выбрать Россети Центр',
		15: 'выбрать Россети Центр и Приволжье(ГК)',
		16: 'выбрать Россети Юг(ГК)',
		17: 'выбрать Россети Янтарь',
		18: 'выбрать Магистральные сети'
	]
 ...
 */
def runFilialTestCase(testNum) {
	def testCases = [
		3: 'Отпуск в сеть Филиалы Россети Волга',
		6: 'Отпуск в сеть Филиалы Россети Москвоский регион',
		7: 'Отпуск в сеть Филиалы Россети Северный Кавказ(ГК)',
		8: 'Отпуск в сеть Филиалы Россети Северо-Запад',
		9: 'Отпуск в сеть Филиалы Россети Сибирь(ГК)',
		12: 'Отпуск в сеть Филиалы Россети Урал(ГК)',
		14: 'Отпуск в сеть Филиалы Россети Центр',
		15: 'Отпуск в сеть Филиалы Россети Центр и Приволжье(ГК)',
		16: 'Отпуск в сеть Филиалы Россети Юг(ГК)'
	]
	
	WebUI.callTestCase(findTestCase("Отпуск в сеть сверка/Сверка по филиалам/${testCases[testNum]}"),
 ...
 */
def getTestDataRow(testNum) {
	def rows = [
		0: 87, 1: 76, 2: 81, 3: 73, 4: 82,
		5: 84, 6: 83, 7: 80, 8: 74, 9: 75,
		10: 77, 11: 85, 12: 78, 13: 88,
		14: 71, 15: 72, 16: 79, 17: 86, 18: 89
	]
	return rows[testNum] ?: 0
}
...
static def Check(Map params) {
	try {
		...
		WriteToExcel2(err: "Ошибка сравнения: ${e.getMessage()}", testNum: params.testNum)
		return false
	}
}
...
static def WriteToExcel(Map params) {
	try {
		...
		String dZO = WebUI.getText(findTestObject('Отпуск в сеть(виджеты)/фильтр ДЗО'), FailureHandling.OPTIONAL) ?: ""
		String year = WebUI.getText(findTestObject('Отпуск в сеть(виджеты)/фильтр Дата'), FailureHandling.OPTIONAL) ?: "2025"
		...
		ExcelKeywords.setValueToCellByIndex(sheet, n, 7, params.status ?: "")
...
static def WriteToExcel2(Map params) {
	try {
		...
		String date = new SimpleDateFormat('dd.MM.yyyy').format(d)
		String dZO = WebUI.getText(findTestObject('Отпуск в сеть(виджеты)/фильтр ДЗО'), FailureHandling.OPTIONAL) ?: ""
		String year = WebUI.getText(findTestObject('Отпуск в сеть(виджеты)/фильтр Дата'), FailureHandling.OPTIONAL) ?: ""
